...
<?php
    ...
    $downQuery = mysqli_query($db_connect,"SELECT * FROM downloads WHERE reg_userCartid = '$userId'"); 
    $downNumRows = mysqli_num_rows($downQuery);
    

    if ($downNumRows <= 0 ) {
        $_SESSION['success_flash'] = 'No Beats!';
...
?>
        ...
        <?php  while($downloadBeatFetch = mysqli_fetch_assoc($downQuery)):  ?>
